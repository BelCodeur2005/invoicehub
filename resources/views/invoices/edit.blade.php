...
@section('page-title', 'Modifier la Facture #' . $invoice->number)
@section('content')
<div class="space-y-6">
    <!-- Header -->
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Modifier Facture #{{ $invoice->number }}</h1>
            <p class="mt-2 text-sm text-gray-600">Modifiez les informations de cette facture</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('invoices.show', $invoice) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition-colors inline-flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('invoices.update', $invoice) }}" method="POST" class="space-y-6" x-data="invoiceForm()">
                @csrf
                @method('PUT')
    ...
    @enderror
                    ...
                    <div>
                        ...
                        <textarea id="notes"
                                  ...
                                  placeholder="Notes additionnelles...">{{ old('notes', $invoice->notes) }}</textarea>
                        @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                    <a href="{{ route('invoices.show', $invoice) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition-colors">
                        Annuler
                    </a>
