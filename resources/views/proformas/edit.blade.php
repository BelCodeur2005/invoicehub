@extends('layouts.app')
@section('title', 'Modifier le Proforma #' . $proforma->number)
@section('page-title', 'Modifier le Proforma #' . $proforma->number)
@section('content')
<div class="space-y-6">
    <!-- Header -->
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Modifier le Proforma #{{ $proforma->number }}</h1>
            <p class="mt-2 text-sm text-gray-600">Modifiez les informations du proforma</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('proformas.show', $proforma) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition-colors inline-flex items-center">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Informations du proforma</h2>
        </div>
        <div class="p-6">
            <form action="{{ route('proformas.update', $proforma) }}" method="POST" class="space-y-6" x-data="proformaForm()">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="date" class="block text-sm font-medium text-gray-700 mb-2">
                            Date du proforma <span class="text-red-500">*</span>
                        </label>
                        <input type="date"
                            id="date"
                            name="date"
                            value="{{ old('date', $proforma->date ? \Carbon\Carbon::parse($proforma->date)->format('Y-m-d') : '') }}"
                    ...
                    <div>
                        ...
                        <input type="date"
                            ...
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('valid_until') border-red-500 @enderror"
                            required>
                        @error('valid_until')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea id="notes"
                                  ...
                                  placeholder="Notes additionnelles...">{{ old('notes', $proforma->notes) }}</textarea>
                        @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label for="conditionPaiement" class="block text-sm font-medium text-gray-700 mb-2">
                            Condition de paiement
                        </label>
                        <input type="text"
                               id="conditionPaiement"
                               name="conditionPaiement"
                               value="{{ old('conditionPaiement', $proforma->conditionPaiement) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('conditionPaiement') border-red-500 @enderror"
                    ...
                    <div>
                        <label for="delaiDeploiment" class="block text-sm font-medium text-gray-700 mb-2">
                            Délai de déploiement
                        </label>
                        <input type="text"
                               id="delaiDeploiment"
                               name="delaiDeploiment"
                               value="{{ old('delaiDeploiment', $proforma->delaiDeploiment) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('delaiDeploiment') border-red-500 @enderror"
                               placeholder="Ex: 15 jours ouvrables">
                        @error('delaiDeploiment')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Garantie matériel -->
                    <div>
                        <label for="garantieMateriel" class="block text-sm font-medium text-gray-700 mb-2">
                            Garantie matériel
                        </label>
                        <input type="text"
                               id="garantieMateriel"
                               name="garantieMateriel"
                               value="{{ old('garantieMateriel', $proforma->garantieMateriel) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('garantieMateriel') border-red-500 @enderror"
                               placeholder="Ex: 2 ans constructeur">
                        @error('garantieMateriel')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                    <a href="{{ route('proformas.show', $proforma) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition-colors">
                        Annuler
                    </a>
